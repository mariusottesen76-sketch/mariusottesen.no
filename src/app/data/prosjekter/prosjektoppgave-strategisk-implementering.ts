import { type ProsjektType } from "./predictive-sales-coach";

/** Kort metadata for /prosjekter — full dokumentasjon på /strategisk-ai-implementering */
export const prosjektoppgaveStrategiskImplementering: ProsjektType = {
  id: "prosjektoppgave-strategisk-implementering-2026",
  tittel: {
    no: "Strategisk AI-implementering: Fra case til gjennomførbar plan",
    en: "Strategic AI implementation: From case to executable plan",
  },
  teaser: {
    no: "Fullført prosjektoppgave fra Generative AI for Business ved BI — dokumentert på egen prosjektside.",
    en: "Completed master's project from Generative AI for Business at BI — documented on a dedicated project page.",
  },
  bildeUrl: "/images/strategisk-ai-implementering-hjemmeside.png?v=20260717",
  dato: "2026-05-20",
  visningsDato: "20.05.26",
  bildeHint: {
    no: "Klikk på bildet for å se prosjektoppgaven i større format.",
    en: "Click the image to view the master's project in a larger format.",
  },
  bilderUnderHovedbilde: [
    {
      src: "/images/bi-prosjektforside.png",
      alt: {
        no: "BI – prosjektforside for Strategisk AI-implementering",
        en: "BI – project cover for Strategic AI implementation",
      },
    },
  ],
  innhold: { no: "", en: "" },
};
