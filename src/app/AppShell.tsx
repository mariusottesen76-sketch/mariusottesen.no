...
import { LanguageProvider, useLanguage, LanguageToggle, type Lang } from "./LanguageContext";
import { getTranslation } from "./data/translations";
import BackToTop from "./components/BackToTop";
import MobileNav from "./components/MobileNav";
import { pathForTab } from "./lib/locale-routes";
...
function AppContent({ initialTab = "Profil" }: { initialTab?: string }) {
  ...
  const setActiveTab = useCallback(
    (tab: string) => {
      ...
      router.push(pathForTab(tab, lang));
    },
    [router, lang]
  );
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-slate-200 px-16 sm:px-24 md:px-32 lg:px-40 xl:px-48 2xl:px-56 py-4 md:py-8 relative overflow-x-hidden w-full">
      <div
        className={`${activeTab === "Faginnlegg" || activeTab === "Prosjekter" || activeTab === "Consulting" ? "max-w-7xl" : "max-w-6xl"} mx-auto relative z-10 transition-all duration-500 w-full`}
      >
        <nav className="mb-8 flex w-full items-center justify-between gap-2 border-b border-slate-800/40 pb-3 md:grid md:grid-cols-[auto_minmax(0,1fr)_auto] md:items-center md:gap-x-3">
          ...
          >
            ...
            <div className="hidden items-center gap-0.5 shrink-0 sm:flex md:hidden lg:flex">
              <span className="text-white font-black uppercase text-[9px] sm:text-[11px] md:text-[13px] tracking-tight italic">
                Marius
              </span>
              <span className="text-indigo-500 font-black uppercase text-[9px] sm:text-[11px] md:text-[13px] tracking-tight italic">
                Ottesen
              </span>
            </div>
          </button>
